...
data = pd.read_excel('E:\ArtificialIntelligence&MachineLearning\MachineLearning\Day3\linear4.xlsx')
...
feature_train = data['number_of_seeds'][:-1].values
feature_train = np.reshape(feature_train,(-1,1))
print(feature_train)
plt.plot(feature_train,'b.')

feature_test = data['number_of_fruit'][-1:].values
...
target_train = data['number_of_seeds'][:-1].values
target_train = np.reshape(target_train,(-1,1))
print(target_train)
plt.plot(target_train,'g.')

target_test = data['number_of_fruit'][-1:].values
